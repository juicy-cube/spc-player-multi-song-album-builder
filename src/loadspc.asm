...
; Based on code by Joe Lee
 ...
 LoadSPC:
 	...
 	sta.l	$7f0100
 	lda	#$60		; Bit 6 (MUTE) + Bit 5 (~ECEN) - keeps the DSP fully
 				; silent for the whole duration of the setup (InitDSP,
 				; RAM copy, upload) instead of only blocking echo
 				; writes. Without this the DSP was "live" (unmuted)
 				; while InitDSP was still zeroing out volume etc.
 				; registers, which gave an audible crackle/pop right
 				; after picking a song. The song's own target FLG
 				; value (normally without MUTE) is restored anyway
 				; at the end by spc700_init_code.
 	sta.l	$7f0101
 	sendMusicBlockM $7f, $0100, $00f2, $0002
	...
	; Copy the SPC RAM dump to SNES RAM
 	...
 	; NOTE: EDL/ESA live in the per-song DSP register block (128 bytes per
 	; song, starting at SPC_REG_ADDR+$4000), so they must be indexed by
 	; spcSongNr*128 - the original code hardcoded offset $407d/$406d here,
 	; which only happened to be correct for song 0.
 	sep	#A_8BIT
 	lda	spcSongNr
 	rep	#A_8BIT
 	and	#$00FF
 	...
 	asl	a
 	asl	a
 	asl	a				; a = spcSongNr * 128
 	tax
 	sep	#A_8BIT
 	lda.l	SPC_REG_ADDR+$407d,x	; EDL (song-indexed)
 	beq	+			
 	lda.l	SPC_REG_ADDR+$406d,x	; ESA (song-indexed)
	clc
	adc	#2
 	xba
 	lda	#0
...
InitDSP:
	rep     #XY_8BIT	; xy in 16-bit mode
	sep     #A_8BIT	; make sure A is 8-bit for this read
	lda	spcSongNr

	rep	#A_8BIT		; switch to a 16-bit accumulator
	and	#$00FF		; safely zero the high byte (instead of xba + lda #0)
	asl	a
	asl	a
	asl	a
	asl	a
...
-:
 ...
 MakeSPCInitCode:
	...
	; EDL and FLG live in the per-song DSP register block (128 bytes per
	; song, at SPC_REG_ADDR+$4000) rather than the 8-byte small-register
	; block that x currently indexes, so they need their own
	; spcSongNr*128 index. The original code hardcoded offset $407d/$406c
	; here, which only happened to be correct for song 0.
	php
	rep	#XY_8BIT
	phx				; save x = spcSongNr*8
	sep	#A_8BIT
	lda	spcSongNr
	rep	#A_8BIT
	and	#$00FF
	...
	lda.l	SPC_REG_ADDR+$407d,x	; EDL (song-indexed)
	sta.w	$0019,y
	plx				; restore x = spcSongNr*8
	plp				; restore original flags/widths
	...
	tax
	sep	#A_8BIT
	lda.l	SPC_REG_ADDR+$406c,x	; FLG (song-indexed)
	sta.w   $0025,y
	plx				; restore x = spcSongNr*8
	plp				; restore original flags/widths
